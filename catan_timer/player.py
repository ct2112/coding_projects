	...
	def display_thread(self):
		...
		while 1:
			...
			for player in self.players:
				self.scr.addstr(player.id * 2 + 2, 0, str(player))
			self.scr.refresh()
			time.sleep(1)
	...
	def timer_func(self):
		...
		while 1:
			...
			if self.in_progress:
				...
				if self.players[self.cur_turn].time_left <= 0:
					self.players.remove(self.players[self.cur_turn])
					if self.cur_turn == len(self.players):
						self.cur_turn = 0
					else:
						self.cur_turn = (self.cur_turn + 1) % len(self.players)
			time.sleep(1)
